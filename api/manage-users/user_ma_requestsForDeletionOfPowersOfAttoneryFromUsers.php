...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "proces_delete_poderes") {
        require_once '../../interbase/tmfUser.php';
            ...
            foreach ($arrDataPod as $keyA => $valueA) {
                ...
                if ($boolHasAccesosPod) {
                    ...
                    foreach ($valueA["accesos"] as $keyB => $valueB) {
                        //print_r($valueB);
                        //DELETE  PRODUCTOS PODERES PRODS
                        header('Content-Type: application/json');
                        $var_consulta = "DELETE FROM vus_usuarios_poderes_prods WHERE vus_upr_poder_id = " . $valueB["vus_solupr_poder_id"] . " AND vus_upr_id = $userId";
                        $val = 1;
                        ...
                        echo "delete poderes prod";
                    }
                }

                //DELETE PRODUCTOS PODERES
                header('Content-Type: application/json');
                $var_consulta = "DELETE FROM vus_usuarios_poderes WHERE vus_upp_poder_id = " . $valueA["vus_solupp_poder_id"] . " AND vus_upp_id = $userId";                
                $val = 1;
                if (pg_query($rmfUser, $var_consulta)) {
                    $arrInfo['status'] = $val;
                } else {
                ...
                $oldfile = $target_path . $valueA["vus_solupp_poder_file"];

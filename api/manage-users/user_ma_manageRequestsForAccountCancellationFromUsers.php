...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    $fechaIng = date('Y-m-d h:i:sa');
    $usuario =  $_SESSION['adm_usr_ci'];

    $strTipoValidacion = isset($_REQUEST["validaciones"]) ? $_REQUEST["validaciones"] : '';

     if ($strTipoValidacion == "proces_update_solicitudes") {
        if($estatus == 1){ header('Content-Type: application/json');
            $status = 1;
    ...
    } else  if ($strTipoValidacion == "proces_update_usuario") {
        $status = 2;

        header('Content-Type: application/json');
        $var_consulta = "UPDATE vus_usuarios SET vus_usr_estatus = '$status',vus_usr_obs = '$obs', vus_usr_sta = '2',vus_usr_usr = '$usuario' ,vus_usr_dt = '$fechaIng' WHERE vus_usr_id = $userId";
            ...
            <table id="tableData" class="table table-bordered table-striped table-hover table-sm">
                ...
                <tbody>
                    ...
                    ?>
                            ...
                            <input type="hidden" name="hid_tpr_g<?php print $intContador; ?>" id="hid_tpr_g<?php print $intContador; ?>" value="<?php echo  $rTMP["value"]['vus_solpro_tpr_id']; ?>">
        ...
        ?>
                <div class="col-sm-6">
                    <div class="form-check" style="text-align: left !important;">
                        <input type="hidden" value="1" name="<?php echo  $rTMP["value"]['ctg_tpr_desc']; ?>" id="<?php echo  $rTMP["value"]['ctg_tpr_desc']; ?>" class="form-check-input">

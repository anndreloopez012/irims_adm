<script>
    ...
    function fntUpdate() {
        ...
        alertify.confirm('AVISO', 'Seguro que desea continuar? ', function() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "ma_paymentRightsFromUsers.php?ajax=true&validaciones=proces_update",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            $('#formData')[0].reset();
    ...
    function fntUpdatePay() {
            ...
            $.ajax({
                ...
                data: datos,
                dataType: 'json',
                url: "ma_paymentRightsFromUsers.php?ajax=true&validaciones=proces_pay",
                success: function(r) {
                    if (r.status) {
                        if (r.status == 1) {
                            //alertify.alert('AVISO', 'Datos cargados correctamente');
    ...
    function fntDibujoTabla() {
        ...
        $.ajax({
            ...
            url: "ma_paymentRightsFromUsers.php?validaciones=busqueda_tabla",
            data: {
    ...
    function fntDibujoCompany() {
        ...
        $.ajax({
            ...
            url: "ma_paymentRightsFromUsers.php?validaciones=data_company",
            data: {
                rif: strRif,
            },
            async: true,
    ...
    function fntDibujoDowload() {
        ...
        $.ajax({
            ...
            url: "ma_paymentRightsFromUsers.php?validaciones=data_dowload",
            data: {
                codigo_des: strCodigo,
                CodId: strCodId,
            },
            async: true,
            global: false,

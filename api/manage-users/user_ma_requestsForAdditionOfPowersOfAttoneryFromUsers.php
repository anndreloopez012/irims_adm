...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "proces_insert_poderes") {
        require_once '../../interbase/tmfUser.php';
        ...
        if (is_array($arrDataPod) && (count($arrDataPod) > 0)) {
            ...
            foreach ($arrDataPod as $keyA => $valueA) {
                ...
                $var_consulta = "INSERT INTO vus_usuarios_poderes (vus_upp_id,vus_upp_tpr_id,vus_upp_poder_id,vus_upp_poder_file,vus_upp_poder_file_ubi,vus_upp_rif,vus_upp_sta,vus_upp_usr,vus_upp_dt) VALUES ($userId,'" . $valueA["vus_solupp_tpr_id"] . "','" . $valueA["vus_solupp_poder_id"] . "','" . $valueA["vus_solupp_poder_file"] . "','" . $valueA["vus_solupp_poder_file_ubi"] . "','" . $valueA["vus_solupp_rif"] . "','$estatus','$usuario','$fechaIng')";
                $val = 1;
                ...
                echo "insert poderes <br>";

                //enviar correo de solicitud autorizado

                 //MOVIMIENTO DE ARCHIVOS FISICOS
                 $adm_root = "/var/www/html/$location_adm/";
                 $vus_root = "/var/www/html/$location_vus/";

                 $target_path = $adm_root . "docs/usuarios/";
                 @mkdir($target_path, 7777);
                 shell_exec("chmod 7777 $target_path");

                 $target_path = $adm_root . "docs/usuarios/" . $userId . "/";
                 @mkdir($target_path, 7777);
                 shell_exec("chmod 7777 $target_path");

                 $ubicacion = $valueA["vus_solupp_poder_file_ubi"];

                 $oldfile = $vus_root . $ubicacion . $valueA["vus_solupp_poder_file"];
                 $newfile = $target_path . $valueA["vus_solupp_poder_file"];

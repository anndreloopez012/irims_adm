...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    $num_sol = isset($_POST["num_sol"]) ? $_POST["num_sol"]  : '';
    ...
    } elseif ($strTipoValidacion == "proces_pay") {
        require_once '../../interbase/tmfUser.php';
        $status = 2;
        $arrData = array();
        $var_consulta = "SELECT a.* ,b.*
        FROM  vus_usuarios_solicitudes a 
        RIGHT JOIN vus_usuarios_solicitudes_pagos  b
        ON a.vus_sol_num = b.vus_solpag_num
        WHERE a.vus_sol_num = '$num_sol'";
        //print_r($var_consulta);
        $qTMP = pg_query($rmfUser, $var_consulta);
        while ($rTMP = pg_fetch_assoc($qTMP)) {
            //USUARIOS SOLICITUDES
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_num"]                 = $rTMP["vus_sol_num"];
            $arrData[$rTMP["vus_sol_num"]]["vus_usr_id"]                 = $rTMP["vus_usr_id"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_tipo"]                 = $rTMP["vus_sol_tipo"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_fec"]                 = $rTMP["vus_sol_fec"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_can"]                 = $rTMP["vus_sol_can"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_canobs"]                 = $rTMP["vus_sol_canobs"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_pagrec"]                 = $rTMP["vus_sol_pagrec"];
            $arrData[$rTMP["vus_sol_num"]]["ctg_ban_id"]                 = $rTMP["ctg_ban_id"];
            $arrData[$rTMP["vus_sol_num"]]["ctg_ban_name"]                 = $rTMP["ctg_ban_name"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_pagmonto"]                 = $rTMP["vus_sol_pagmonto"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_pagnamepdf"]                 = $rTMP["vus_sol_pagnamepdf"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_paglocpdf"]                 = $rTMP["vus_sol_paglocpdf"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_obs"]                 = $rTMP["vus_sol_obs"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_estatus"]                 = $rTMP["vus_sol_estatus"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_sta"]                 = $rTMP["vus_sol_sta"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_usr"]                 = $rTMP["vus_sol_usr"];
            $arrData[$rTMP["vus_sol_num"]]["vus_sol_dt"]                 = $rTMP["vus_sol_dt"];
            //USUARIOS SOLICITUDES PAGOS
            $arrData[$rTMP['vus_sol_num']]['accesos'][$rTMP['vus_solpag_tpr_id']]['vus_solpag_num']        = $rTMP['vus_solpag_num'];
            $arrData[$rTMP['vus_sol_num']]['accesos'][$rTMP['vus_solpag_tpr_id']]['vus_solpag_tpr_id']        = $rTMP['vus_solpag_tpr_id'];
            $arrData[$rTMP['vus_sol_num']]['accesos'][$rTMP['vus_solpag_tpr_id']]['vus_solpag_pagval']        = $rTMP['vus_solpag_pagval'];
            $arrData[$rTMP['vus_sol_num']]['accesos'][$rTMP['vus_solpag_tpr_id']]['vus_solpag_pagcuo']        = $rTMP['vus_solpag_pagcuo'];
            $arrData[$rTMP['vus_sol_num']]['accesos'][$rTMP['vus_solpag_tpr_id']]['vus_solpag_sta']        = $rTMP['vus_solpag_sta'];
            $arrData[$rTMP['vus_sol_num']]['accesos'][$rTMP['vus_solpag_tpr_id']]['vus_solpag_usr']        = $rTMP['vus_solpag_usr'];
            $arrData[$rTMP['vus_sol_num']]['accesos'][$rTMP['vus_solpag_tpr_id']]['vus_solpag_dt']        = $rTMP['vus_solpag_dt'];
            $arrData[$rTMP['vus_sol_num']]['accesos'][$rTMP['vus_solpag_tpr_id']]['vus_solpag_tipo']        = $rTMP['vus_solpag_tipo'];
        }
        pg_free_result($qTMP);
            ...
            foreach ($arrData as $keyA => $valueA) {
                ...
                if ($boolHasAccesosPod) {
                    ...
                    foreach ($valueA["accesos"] as $keyB => $valueB) {
                        ...
                        //DELETE PODERES
                        header('Content-Type: application/json');
                        $day = 365 * $valueB["vus_solpag_pagcuo"];
                        $countDays = date("Y-m-d", strtotime("$day days"));

                        $var_consulta = "UPDATE vus_usuarios_productos SET vus_usp_ven =' $countDays',vus_usp_sta = '$status',vus_usp_usr = '$usuario',vus_usp_dt = '$fechaIng' WHERE vus_usp_id = '$userId'AND vus_usp_tpr_id =  '" . $valueB["vus_solpag_tpr_id"] . "'";
                        $val = 1;
                        if (pg_query($rmfUser, $var_consulta)) {
                            $arrInfo['status'] = $val;
                        } else {
    ...
    ?>
        ...
        if (is_array($arrData) && (count($arrData) > 0)) {
            ...
            foreach ($arrData as $keyA => $valueA) {
                ...
                $fecsol = $valueA['vus_sol_fec'];
                $year = date("Y", strtotime($fecsol));
                $month = date("m", strtotime($fecsol));

                $urlDoc =  $ipServer . "/" . $location_vus . "/docs/pago_derechos/" . $year . "/" . $month . "/" . $CodId . "/" . $valueA['vus_sol_pagnamepdf'];

                $total = 0;
        ?>
                <div class="col-12 row">

                    <label for="Code" class="col-sm-5 col-form-label">Bank code</label>
                    <div class="col-sm-1">
                        <input class="form-control form-control-sm" name="ctg_ban_id" id="ctg_ban_id" type="text" value="<?php echo  $valueA['ctg_ban_id']; ?>">
                    </div>
                    <div class="col-sm-3">
                        <input class="form-control form-control-sm" name="ctg_ban_name" id="ctg_ban_name" type="text" value="<?php echo  $valueA['ctg_ban_name']; ?>">
                    </div>
                    <label for="Code" class="col-sm-5 col-form-label">Bank deposit number</label>
                    <div class="col-sm-2">
                        <input class="form-control form-control-sm" name="vus_sol_pagrec" id="vus_sol_pagrec" type="text" value="<?php echo  $valueA['vus_sol_pagrec']; ?>">
                    </div>
                    <div class="col-3">
                        <a href="<?php echo  $urlDoc ?>" target="blank" title="137_2020-03_poder_1.pdf"><i class="fad fa-2x fa-file-pdf"></i></a>
                    </div>
                    <label for="Code" class="col-sm-5 col-form-label">Payment remarks</label>
                    <div class="col-sm-5">
                        <textarea class="form-control form-control-sm " name="vus_sol_canobs" id="vus_sol_canobs" rows="3"><?php echo  $valueA['vus_sol_canobs']; ?></textarea>
                    </div>
                    <div class="col-12">
                        <table id="tableData" class="table table-hover table-dark table-sm">
                            <thead>
                                <tr>
                                    <th WIDTH="25%" scope="col" style="background: #383f45">UNIT</th>
                                    <th WIDTH="25%" scope="col" style="background: #383f45">Cost per use rights</th>
                                    <th WIDTH="25%" scope="col" style="background: #383f45">You pay fees</th>
                                    <th WIDTH="25%" scope="col" style="background: #383f45">Subtotal</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                ...
                                ?>
                                        <tr id="licence">
                                            <td WIDTH="25%"><?php echo $valueB['vus_solpag_tpr_id']; ?></td>
                                            <td WIDTH="25%"><?php echo $valueB['vus_solpag_pagval']; ?></td>
                                            <td WIDTH="25%"><?php echo $valueB['vus_solpag_pagcuo']; ?></td>
                                            <td WIDTH="25%"><?php echo $valueB['vus_solpag_pagval'] * $valueB['vus_solpag_pagcuo']; ?></td>
                                            <?php $total = $total + ($valueB['vus_solpag_pagval'] * $valueB['vus_solpag_pagcuo']); ?>
                                        </tr>
                                <?php
                                    }
                                }
                                ?>
                                <tr>
                                    <td colspan="3" aling="right">Total amount paid</td>
                                    <td aling="right">

    ...
    $strTipoValidacion = isset($_REQUEST["validaciones"] )?$_REQUEST["validaciones"] :'';
        ...
        if ( $strTipoValidacion == "insert" ){
            ...
            if (empty($id)) {
                ...
                $var_consulta="SELECT system_06010103($insert,'$numOrder','$description','$comments','$link','$usuario',$status,'$fechaIng',$options,$idMax,$code,$categori)";
                $val = 1;
                ...
                header('Content-Type: application/json');
                $var_consulta="SELECT system_06010103($update,'$numOrder','$description','$comments','$link','$usuario',$status,'$fechaIng',$options,$idMax,$code,$categori)";
                $val = 1;
                if (pg_query($rmfAdm, $var_consulta)) {
                    $arrInfo['status'] = $val;
                } else {
            ...
            header('Content-Type: application/json');
            $var_consulta="SELECT system_06010103($replace,'$numOrder','$description','$comments','$link','$usuario',$status,'$fechaIng',$options,$idMax,$code,$categori)";
            $val = 1;
            if (pg_query($rmfAdm, $var_consulta)) {
                $arrInfo['status'] = $val;
            } else {
            ...
            header('Content-Type: application/json');
            $var_consulta="SELECT system_06010103($delete,'$numOrder','$description','$comments','$link','$usuario',$status,'$fechaIng',$options,$idMax,$code,$categori)";
            $val = 1;
            if (pg_query($rmfAdm, $var_consulta)) {
                $arrInfo['status'] = $val;
            } else {
...
//DIBUJO DE TABLA PRODUCTOS MENU_l
            ...
            ?>
            <tbo class="col-md-12 tableFixHead">

...
function fntValidacionCode(){
    ...
    if ( !isNaN(intCode) && (intCode>0) ){
        ...
        $.ajax({
            ...
            url: "mu_ss_menus.php?validaciones=validacion_code&code="+intCode,
            async: true,
            global: false,
    ...
    var datos=$('#formData').serialize();
       ...
       //return false;
        ...
        $.ajax({
            ...
            data:datos,
            dataType: 'json',
            url:"mu_ss_menus.php?ajax=true&validaciones=insert",
            success:function(r){
              if( r.status ){
                  if( r.status==1){
  ...
  alertify.confirm('Status Of Products Type ', 'Are you sure you wish to continue? ', function(){ 
    var datos=$('#formData').serialize();
      ...
      $.ajax({
          ...
          data:datos,
          dataType: 'json',
          url:"mu_ss_menus.php?ajax=true&validaciones=replace",
          success:function(r){
            if( r.status ){
                  if( r.status==1){
                    alertify.alert('Status Of Products Type ', 'Data successfully updloaded');
  ...
  alertify.confirm('Status Of Products Type ', 'Are you sure you wish to continue?  ', function(){ 
    ...
    //return false;
      ...
      $.ajax({
          ...
          data:datos,
          dataType: 'json',
          url:"mu_ss_menus.php?ajax=true&validaciones=delete",
          success:function(r){
            if( r.status ){
                  if( r.status==1){
                    alertify.alert('Status Of Products Type ', 'Data successfully updloaded');

...
if ( isset($_GET["validaciones"]) && !empty($_GET["validaciones"]) ){
    ...
    $rs = pg_query("SELECT count(*)FROM  ctg_tipo_productos_estatus");
    ...
    $strTipoValidacion = isset($_REQUEST["validaciones"] )?$_REQUEST["validaciones"] :'';
        ...
        if ( $strTipoValidacion == "insert" ){
            ...
            if (empty($id)) {
                ...
                $var_consulta="SELECT system_06011007($insert,'$code','$description','$comments','$unit','$status','$usuario',
                '$fechaIng',$id,$idMax)";
                $val = 1;
                ...
                $var_consulta="SELECT system_06011007($update,'$code','$description','$comments','$unit','$status','$usuario','$fechaIng',$id,$idMax)";
                $val = 1;
                if (pg_query($rmfAdm, $var_consulta)) {
                    $arrInfo['status'] = $val;
                } else {
        ...
        if ( $strTipoValidacion == "replace" ){
            header('Content-Type: application/json');
            $var_consulta="SELECT system_06011007($replace,'$code','$description','$comments','$unit','$status','$usuario','$fechaIng',$id,$idMax)";
            $val = 1;
            if (pg_query($rmfAdm, $var_consulta)) {
                $arrInfo['status'] = $val;
            } else {
        ...
        if ( $strTipoValidacion == "delete" ){
            header('Content-Type: application/json');
            $var_consulta="SELECT system_06011007($delete,'$code','$description','$comments','$unit','$status','$usuario','$fechaIng',$id,$idMax)";
            $val = 1;
            if (pg_query($rmfAdm, $var_consulta)) {
                $arrInfo['status'] = $val;
            } else {
...
//DIBUJO DE TABLA PRODUCTOS CATEGORIAS
        if ( $strTipoValidacion == "busqueda_categoria" ){
            ...
            $strFilter = "";
            if ( !empty($strSearch) ){
                $strFilter = " AND ( UPPER(ctg_tps_desc) LIKE '%{$strSearch}%' ) ";
            }
            
            $arrFields= array();
            $var_consulta= "SELECT ctg_id, ctg_tps_id, ctg_tps_desc, ctg_tps_obs  
                            FROM ctg_tipo_productos_estatus  
            ...
            while ( $rTMP = pg_fetch_assoc($qTMP) ){
                ...
                $arrFields[$rTMP["ctg_id"]]["ctg_tps_id"]                 = $rTMP["ctg_tps_id"];
                $arrFields[$rTMP["ctg_id"]]["ctg_tps_desc"]                 = $rTMP["ctg_tps_desc"];
                $arrFields[$rTMP["ctg_id"]]["ctg_tps_obs"]                 = $rTMP["ctg_tps_obs"];
            ...
            <tbody>
                ...
                ?> 
                    <tr data-dismiss="modal" style="cursor:pointer;" onclick="fntSelect('<?php print $intContador; ?>');">
                        <td><?php echo  $rTMP["value"]['ctg_tps_id']; ?></td>
                        <td><?php echo  $rTMP["value"]['ctg_tps_desc']; ?></td>
                    </tr>
                        <input type="hidden" name="hidId_<?php print $intContador; ?>" id="hidId_<?php print $intContador; ?>" value="<?php echo  $rTMP["value"]['ctg_id']; ?>">
                        <input type="hidden" name="hidCode_<?php print $intContador; ?>" id="hidCode_<?php print $intContador; ?>" value="<?php echo  $rTMP["value"]['ctg_tps_id']; ?>">
                        <input type="hidden" name="hidDescription_<?php print $intContador; ?>" id="hidDescription_<?php print $intContador; ?>" value="<?php echo  $rTMP["value"]['ctg_tps_desc']; ?>">
                        <input type="hidden" name="hidComment_<?php print $intContador; ?>" id="hidComment_<?php print $intContador; ?>" value="<?php echo  $rTMP["value"]['ctg_tps_obs']; ?>">
                <?PHP
                    $intContador++;                    
                  }
                }
        ...
        if ( $strTipoValidacion == "validacion_code" ){
            ...
            if ($intData){
                ...
                $var_consulta ="SELECT * FROM ctg_tipo_productos_estatus WHERE ctg_tps_id  = '$intData'";  
                    if (pg_query($rmfAdm, $var_consulta)) {
                        $boolExiste = true;
                    } else {
